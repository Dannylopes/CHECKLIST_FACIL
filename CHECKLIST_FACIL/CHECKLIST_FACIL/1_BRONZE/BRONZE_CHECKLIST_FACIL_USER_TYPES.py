...
# MAGIC %md
# MAGIC ### *Notebook de Ingestão da tabela '**checklistfacil_user_types**'*
# MAGIC | Camada      | Domínio     | Entidade    |
# MAGIC | ----------  | ----------  | ----------  |
# MAGIC | Corp     | cvc_corp_work (Bronze)     | checklistfacil_user_types     |
# MAGIC
# MAGIC
...
TABLE_NAME = f"cvc_corp_work.checklistfacil_{endpoint}".replace('-', '_')
LOAD_TYPE = "FULL"
PAGE_SIZE = 100
...
df_final = spark.sql(f"SELECT * FROM {TABLE_NAME}")
display(df_final)