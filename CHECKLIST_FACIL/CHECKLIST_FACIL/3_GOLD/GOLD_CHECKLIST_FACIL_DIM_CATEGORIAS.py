...
# MAGIC %md
# MAGIC ### *Notebook da tabela Gold '**checklistfacil_dim_categorias**'*
# MAGIC | Camada      | Domínio     | Entidade    |
# MAGIC | ----------  | ----------  | ----------  |
# MAGIC | Corp     | cvc_corp       | checklistfacil_dim_categorias  |
# MAGIC
# MAGIC
...
TABLE_SILVER = 'checklistfacil_categories'
TABLE_GOLD   = 'checklistfacil_dim_categorias'
TABLE_PK = 'categoryId'
...
# from pyspark.sql.functions import col, crc32, current_timestamp, from_utc_timestamp

df_gold = df_without_dt_hr_carga.select(
        crc32(col("categoryId").cast("string")).alias("sk_categoria"),
        col("categoryId")        .alias("id_original_categoria"),
        col("checklistId")       .alias("id_checklist"),
        col("parentId")          .alias("id_categoria_pai"),
        col("type")              .alias("tipo_categoria"),
        col("name")              .alias("nome_categoria"),
        col("description")       .alias("descricao_categoria"),
